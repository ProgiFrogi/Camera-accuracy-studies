...
#include <float.h>
#include <iostream>
#include <lapacke.h>  //matlab 
//#include "include/lapacke_config.h"  //LAPACK manual, not successful
...
namespace Constants {
const int32_t L2gradient = 1<<31;//CV_CANNY_L2_GRADIENT
}
 ...
 */
//Get the absolute value of the angle between two radian angles
...
rect_iter * ri_ini(struct rect * r)
{
  ...
  //Starting from the starting point (x1, y1) of the line segment, reconstruct the four vertices of the rectangle in a counterclockwise direction from one end
...
//For the pixels that make up the arc, mark the polarity. If the direction of the gradient and the direction of the arc are consistent, it is SAME_POLE; otherwise, it is OPP_POLE. This mark is initially 0
#define NOTDEF_POL 0
#define SAME_POL 1
#define OPP_POL  -1
/*----------------------------------------------------------------------------*/
                                      ...
                                      double sigma_scale )
{
  ...
  N = (unsigned int) ceil( in->xsize * scale );//Round up
   ...
   */
  prec = 3.0;//The outer edge of the Gaussian kernel drops to 10^(-3)
 ...
 */
//Return an angle image angles with the gradient angle rotated 90° clockwise, if the gradient angle is (gx,gy)->(-gy,gx),
//and the gradient modulus image modgrad, then perform pseudo-sorting based on n_bins and return the head pointer list_p of the linked list, which stores the coordinates
                              ...
                              image_double * modgrad, unsigned int n_bins )
{
  ...
  struct coorlist ** range_l_s; /*Array of pointers to the start of the bin list, representing an array of 1024 bin head pointers*/
  struct coorlist ** range_l_e; /*Array of pointers to the end of the bin list, representing an array of 1024 bin tail pointers*/
 ...
 // if( list == NULL || range_l_s == NULL || range_l_e == NULL )
    ...
    for(y=0;y<n-1;y++)
      {
        ...
        if( range_l_e[i] == NULL )
          range_l_s[i] = range_l_e[i] = temp;//Record the head pointer of the i-th region to range_l_s[i]
        else
          {
            range_l_e[i]->next = temp;//The i-th region is completed by the tail pointer range_l_e[i]
            range_l_e[i] = temp;
          }
        range_l_e[i]->x = (int) x;//Record the coordinates (x, y) to the i-th partition
        range_l_e[i]->y = (int) y;
   ...
   */
  for(i=n_bins-1; i>0 && range_l_s[i]==NULL; i--);//Find the first non-empty partition bin
  start = range_l_s[i];
  end = range_l_e[i];
  if( start != NULL )
    while(i>0)
                         ...
                         image_double modgrad, double reg_angle, double prec )
{
  ...
  if( double_equal(Ixx,0.0) && double_equal(Iyy,0.0) && double_equal(Ixy,0.0) )//Determine if Ixx, Iyy, Ixy are very close to 0, since they are of type double, a special function is needed to determine this
  ...
  if( temp1 > prec )//This is because the smaller eigenvalue of the two orthogonal axes of the region's inertia matrix may differ by 180° from the region's angle
  {
	  //------------------------------------------
	  //theta += M_PI;   //origin code
	  //------------------------------------------
	  //------------------------------------------
	  //My code, added this segment to limit theta to the range (-pi, pi)
	  //int flag = 0;
                         ...
                         double prec, double p, struct rect * rec )
{
  ...
  /* theta */
  //Use the inertia matrix to obtain a more precise angle estimate
  ...
  /* length and width:
     ...
     in the region.
   */
  //Because the region's direction vector is (dx, dy)
  /*
  ------------------->x
  |\
  | \
 ...
 \|/
  y
  Therefore, rotating 90° clockwise is (-dy, dx)
  */
  l_min = l_max = w_min = w_max = 0.0;
  for(i=0; i<reg_size; i++)//Use the dot product of vectors to find the projections in the direction of the line segment and the direction perpendicular to the line segment to find l, w
    {
...
//The region's centroid and angle have already been calculated, so only rectangular approximation is performed. However, region2rect also performs centroid and angle calculation.
					...
					double reg_theta,double prec, double p, struct rect * rec )
{
  ...
  //Obtain the region's direction vector (dx, dy)
  dx = cos(reg_theta);
  dy = sin(reg_theta);
  l_min = l_max = w_min = w_max = 0.0;
  for(i=0; i<reg_size; i++)//Use the dot product of vectors to find the projections in the direction of the line segment and the direction perpendicular to the line segment to find l, w
                                 ...
                                 double density_th )
{
  ...
  /* compute region point2is density */ //This density judgment has already been judged outside the function, so it should not need to be judged again, right?
      ...
      if( dist( xc, yc, (double) reg[i].x, (double) reg[i].y ) < rec->width )
        {
          ...
          sum += ang_d;//Add the angle difference
          s_sum += ang_d * ang_d;//Add the square of the angle difference
          ++n;
        }
    }
  mean_angle = sum / (double) n;
  //Use twice the standard deviation as the new angle tolerance, initially 22.5° * pi / 180
  tau = 2.0 * sqrt( (s_sum - 2.0 * mean_angle * sum) / (double) n  +  mean_angle*mean_angle ); /* 2 * standard deviation */
  //Perform region growth again with the new angle tolerance
                         ...
                         double prec, double p, rect * rect_up, rect * rect_down)
{
	...
	for ( i = 0; i < reg_size; i++)
	{
		...
		{
			case SAME_POL: same_pol_cnt++;break;//Count the number of same-polarity pixels
			case OPP_POL : opp_pol_cnt++; break;//Count the number of opposite-polarity pixels
			default:break;
		}
	 //Select the angle direction as the normal direction, and the line equation passing through the centroid (x-xi) * dx + (y-yi) * dy = 0. Then, for points with the same direction as theta, substituting into the equation gives the distance d. If d >= 0, it is assigned to reg_up; if d < 0, it is assigned to reg_down
	...
	//For regions that have already been marked with polarity, we do not need to perform polarity analysis again
	...
	//Calculate the centroid of the upper half of the region with the same main direction
	reg_up_x = reg_up_y = 0;
	//sum = 0;
	...
	//Calculate the centroid of the lower half of the region with the main direction
	...
	//Estimate the directions of the two regions
	...
	//Rotate to 0° to compare theta, reg_up_theta, reg_down_theta
	...
	if(temp2>= M_PI/2 || temp2 <= -M_PI/2)
		temp2 += 0;*/
	//if(temp1 >= prec/10 && temp2 <= -prec/10)//clockwise, the gradient direction of the edge and the direction of the arc pointing to the center are opposite, polarity = -1
	if(temp1 >= M_1_8_PI/10 && temp2 <= -M_1_8_PI/10)//Experiments show that using fixed values gives better results
	{
		main_rect->polarity = -1;
		rect_up->polarity = -1;
	    rect_down->polarity = -1;
	...
	//else if(temp1 <= -prec/10 && temp2 >= prec/10)//counterclockwise, the gradient direction of the edge and the direction of the arc pointing to the center are the same, polarity = 1
	else if(temp1 <= -M_1_8_PI/10 && temp2 >= M_1_8_PI/10)//Experiments show that using fixed values gives better results
	{
		main_rect->polarity = 1;
		rect_up->polarity = 1;
	    rect_down->polarity = 1;
	...
	{
		//Already set to USED in region_grow
		//for ( i = 0; i< reg_size; i++)
                               ...
                               int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  image_char used;
  image_char pol;  //For the pixels that make up the arc, mark the polarity. If the direction of the gradient and the direction of the arc are consistent, it is SAME_POLE; otherwise, it is OPP_POLE. This mark is initially 0
...
//  double log_nfa1,log_nfa2;
  ...
  if( scale != 1.0 )
    {
	  //Perform Gaussian downsampling of the image according to the scale. Note that the width and height are rounded up. Set the downsampled height and width to imgN * imgM
  ...
  xsize = angles->xsize;//The x size of the downsampled image, width imgM
  ysize = angles->ysize;//The y size of the downsampled image, height imgN
  ...
  */
  logNT = 5.0 * ( log10( (double) xsize ) + log10( (double) ysize ) ) / 2.0
          + log10(11.0);
  min_reg_size = (int) (-logNT/log10(p)); /*Minimum number of align points in each rectangular region, the minimum number of align point2is in each rectangular region*/
  /* initialize some structures */
  if( reg_img != NULL && reg_x != NULL && reg_y != NULL ) /* save region data */
    region = new_image_int_ini(angles->xsize,angles->ysize,0);//Allocate int type memory of the same size as the downsampled image. This memory is used to mark the detected line segment index to the corresponding image grid. This part can be omitted.
  used = new_image_char_ini(xsize,ysize,NOTUSED);//Allocate char type memory of the same size as the downsampled image.
  pol  = new_image_char_ini(xsize,ysize,NOTDEF_POL);//Polarity mark of the pixel point where the gradient and the arc direction are located
  reg = (struct point2i *) calloc( (size_t) (xsize*ysize), sizeof(struct point2i) );
  if( reg == NULL ) error("not enough memory!");

  list_p_temp = list_p;//Record the head pointer of the linked list, which will be used for memory release later
  ...
  for(; list_p_temp != NULL; list_p_temp = list_p_temp->next )
    if( used->data[ list_p_temp->x + list_p_temp->y * used->xsize ] == NOTUSED &&
          ...
          because we are only interested in the exact NOTDEF value */
      {
		...
		//reg is a one-dimensional point2i array of length imgN * imgM, with enough space to store the growing region. reg_size is the number of data stored in it, recording the point2i of the region.
		//reg_angle is the double variable of the main direction of this region, and the stored angle is in radians.
		  seed_cnt ++;
		...
		//Obtain the parameters of the approximate outer rectangle based on the grown region. The rectangle parameters include: starting point, ending point, direction theta, width, etc.
        ...
        //Purify, improve the region's density threshold by regrowing the region
        if( !refine( reg, &reg_size, modgrad, reg_angle,
                     prec, p, &main_rect, used, angles, density_th ) ) continue;

		refine_cnt++;
        // compute NFA value 
        log_nfa = rect_improve(&main_rect,angles,logNT,log_eps);//Attempt to improve the nfa value by refining the rectangular region
		...
		//------------------------------------------------------------------------------------------------- 
		/*
		cout<<ls_count<<'\t'<<main_rect.theta<<'\t'<<main_rect.theta*180/M_PI<<"\t polarity:"<<main_rect.polarity<<endl;//print theta

		fstream file1,file2;
		if(ls_count == 1)//clear content
		{
		...
		if(ls_count == 1)//keep the region of the first line segment
		{
		...
		*/
		//-------------------------------------------------------------------------------------------------------
        /* add region number to 'region' image if needed */ //Mark the detected line segment index to the corresponding image grid. This part can be omitted.
        if( region != NULL )
          for(i=0; i<reg_size; i++)
  ...
  free( (void *) reg );
//  free( (void *) mem_p );
  //Release the storage of the linked list that stores the coordinates sorted by gradient from largest to smallest in 1024 bins, my code
  //---------------------------------------
...
my code, Alan Lu
Input:
img: One-dimensional double array of the input image, size Y * X, stored row-first. Must have memory before passing in.
X: Columns of the input image
Y: Rows of the input image
Output:
n_out: The number of line segments detected by the LSD algorithm is n, return the return value is n line segments, which is a one-dimensional double array of length 8 * n, with each group of 8 representing x1, y1, x2, y2, dx, dy, width, polarity
reg_img: Output marked region, one-dimensional int array, size reg_y * reg_x. Marks the line segment (1, 2, 3, ..., n) it belongs to at the corresponding pixel position. If the value is 0, it means it does not belong to any line segment.
          If the external is int * region_img, just use &region_img to get the returned marked region, no need to pass in if not needed
reg_x: Columns of the output marked region, no need to pass in if not needed
reg_y: Rows of the output marked region, no need to pass in if not needed
*/
...
//lines: Input n_lines line segments, each line segment has 8 values, storing x1, y1, x2, y2, dx, dy, width, polarity
//lines_num:
//new_lines_num: After rejecting short line segments, new_lines_num line segments, stored at the front, while short line segments will be placed at the back
//The length threshold here is very important: currently 8^2, 14^2
void     rejectShortLines(double * lines, int lines_num, int * new_lines_num )
{
	...
	{
		if( lines[i*8+6] < 10)//Reject short lines, the length threshold is important: 8, 14. Needs to be adjusted later
		{
			for ( int j = 0; j<8; j++)
			{
				temp = lines[i*8+j];
			...
			i--; //Check the length of the exchanged line segment, need to roll back
			shor_lines_num++;
			new_num = lines_num - shor_lines_num;
		}
	}
...
/*----------------------------------------------------------------------------*/
//Input:
//start_angle, end_angle, angle range is (-pi, pi). 
//  pi    ------->x  0
//        |
//        |
//       y\/ pi/2
//polarity: When polarity is 1, it represents the angle from start_angle rotated counterclockwise to end_angle; when polarity is -1, it represents the angle from start_angle rotated clockwise to end_angle;
//Return value: Rotation angle coverage
inline double rotateAngle(double start_angle, double end_angle, int polarity)
{
	double coverage;
	//First, angle1 and angle2 need to be converted to the range 0 ~ 2pi
	if(start_angle < 0) start_angle += M_2__PI;//Limit the angle to the range 0 ~ 2pi
	if(end_angle < 0 ) end_angle += M_2__PI;
	if(polarity == 1)//Polarity is 1
	{
...
//Group line segments by convexity and distance
//lines: Input n_lines line segments, each line segment has 8 values, storing x1, y1, x2, y2, dx, dy, length, polarity
//lines_num:
//Output groups. Each group is a vector<int>
//Note: Remember to manually release the region after using the function.
void groupLSs(double *lines, int line_num, int * region, int imgx, int imgy, vector<vector<int>> * groups)
{
	...
	unsigned char isEnd = 0;//Whether to continue searching
	int currentLine; //Current line segment
	char * label = (char*)calloc(line_num, sizeof(char));
	memset(label,0,sizeof(char)*line_num); //init the label all to be zero
	int * group_up = (int*)malloc(sizeof(int)*line_num);//Allocate enough memory to store the line segments obtained along the line segment direction
	int * group_down = (int*)malloc(sizeof(int)*line_num);//Store the line segments grouped in the opposite direction
	int group_up_cnt,group_down_cnt;
	//coorlist * head,*tail;
	vector<int> group_temp;
	point2d dir_vec1,dir_vec2;
	point2i *votebin = (point2i*)calloc(line_num,sizeof(point2i));//Allocate enough memory to be used for voting. x records the line segment index, y records the number of votes
	int bincnt = 0;
	int xx,yy,temp;
	double start_angle,end_angle,angle_delta;
	for ( int i = 0; i<line_num; i++)
	{
		if( label[i] == 0)//Not grouped yet
		{
			group_up_cnt = group_down_cnt = 0;//Each time a group is found, it needs to be reset to zero
			//Start searching from the head of the i-th line segment, perform grouping, and store the result in group_up
			group_up[group_up_cnt++] = i;//Record the line segment i, note that the line segment is 0 ~ line_num-1
			isEnd = 0;//Reset to zero, indicating that it can still start searching from the current line segment and has not finished
	     	currentLine = i;
			while(isEnd == 0)
			{
				label[currentLine] = 1; //Mark this line segment as grouped
				//head = tail = NULL;
		        bincnt = 0;
				dir_vec1.x = lines[currentLine*8+4];
				dir_vec1.y = lines[currentLine*8+5];
				if ( lines[currentLine*8+7] == 1)//Positive polarity
				{
					//Rotate dir_vec1 counterclockwise by 45°
				...
				{
					//Rotate dir_vec1 clockwise by 45°
					dir_vec2.x = (dir_vec1.x - dir_vec1.y)*0.707106781186548; // sqrt(2)/2 = 0.707106781186548
				    dir_vec2.y = (dir_vec1.x + dir_vec1.y)*0.707106781186548;
				}
				for ( int j = 1; j<=4; j++)
					for ( int k = 1; k<=4; k++)//Search in a 4x4 neighborhood
					{
						xx = (int)(lines[currentLine*8+2]*0.8+j*dir_vec1.x+k*dir_vec2.x);
						yy = (int)(lines[currentLine*8+3]*0.8+j*dir_vec1.y+k*dir_vec2.y);
						if(xx < 0 || xx >= imgx || yy < 0 || yy >= imgy)//Out of bounds
							continue;
						temp = region[yy*imgx+xx];
						if(temp>0)//Represents an area with line segment support, within the range 1 ~ line_num
						{
							region[yy*imgx+xx] = -temp;//Take the negative number to mark
							for (xx = 0; xx<bincnt; xx++)
							{
								if(votebin[xx].x == temp - 1)//If voted before, directly add 1 to the number of votes in the corresponding bin
								{
									votebin[xx].y++;
									break;
								}
							}
							if(xx == bincnt)//If not voted before, add the line segment and record the number of votes as 1
							{
								if(bincnt == line_num)
									error("group ls error1");
								votebin[bincnt].x = temp - 1;
								votebin[bincnt].y = 1;
								bincnt++; //Increment the total number of bins by 1
							}
						}
					}
			    //Find the line segment with the most votes, and it must satisfy a certain number
			    temp = 0;
				for ( int j = 0; j<bincnt; j++)
				{
					...
					{
						temp = votebin[j].y;
						xx = votebin[j].x;//Borrow the xx variable
					}
				}
				if ( temp >= 5 && label[xx] == 0 && lines[8*xx+7] == lines[8*i+7] )//Adjust the parameter value for the experiment
				{
					if(group_up_cnt == line_num)
					   error("group ls error2");
					yy = group_up_cnt-1;//Borrow the yy variable
					...
					if(angle_delta <= M_3_8_PI)//The rotation angle between adjacent line segments must also satisfy within pi/4
					{
						group_up[group_up_cnt++] = xx;//Push the line segment
						currentLine = xx; //Update the current search line segment
					}
					else
						isEnd = 1;
				}
				else
					isEnd = 1;//End, no more line segments can be found for grouping
			}
			//Start searching from the tail of the i-th line segment, perform grouping, and store the result in group_down. Note that the i-th line segment is stored in both group_up and group_down at index 0.
			...
			while(isEnd == 0)
			{
				...
				dir_vec1.x = -lines[currentLine*8+4];
				dir_vec1.y = -lines[currentLine*8+5];
				if ( lines[currentLine*8+7] == 1)//Same polarity
				{
					//Rotate dir_vec1 clockwise by 45°
					dir_vec2.x = (dir_vec1.x - dir_vec1.y)*0.707106781186548; // sqrt(2)/2 = 0.707106781186548
				    dir_vec2.y = (dir_vec1.x + dir_vec1.y)*0.707106781186548;
				...
				if ( temp >= 5 && label[xx] == 0 && lines[8*xx+7] == lines[8*i+7])//Adjust the parameter value for the experiment
				{
					if(group_down_cnt == line_num)
					   error("group ls error2");
					yy = group_down_cnt-1;//Borrow the yy variable
					start_angle = atan2(lines[8*group_down[yy]+5],lines[8*group_down[yy]+4]);
					end_angle = atan2(lines[8*xx+5],lines[8*xx+4]);
					angle_delta = rotateAngle(end_angle,start_angle,(int)lines[8*i+7]);//Note that this time it needs to be reversed because the search starts from the tail
					if(angle_delta < M_3_8_PI)//The rotation angle between adjacent line segments must also satisfy within pi/4, pi*3/8 = 66.5°
					{
						group_down[group_down_cnt++] = xx; //Push the line segment
						currentLine = xx; //Update the current search line segment
					}
					else
						isEnd = 1;
			...
			(*groups).push_back(group_temp); //Add line segment grouping
			temp = (*groups).size()-1;
			for (int j = group_down_cnt-1; j>= 0; j--)
			{
				(*groups)[temp].push_back(group_down[j]);
			}
			for (int j = 1; j<group_up_cnt; j++)//Since the i-th line segment is stored in both group_up and group_down, start from index 1
			{
...
//Calculate the span of each group
//Input:
//lines: Input n_lines line segments, each line segment has 8 values, storing x1, y1, x2, y2, dx, dy, length, polarity
//lines_num:
//groups: Groups, each group stores the index of the line segments
//Output:
//coverages: The span of each group. If a group contains only 1 line segment, the span is 0. The length of coverages is equal to the number of groups = groups.size()
//Note: coverages do not need to allocate memory before use, and coverages need to be manually released after use. The length is equal to the number of groups.
void calcuGroupCoverage(double * lines, int line_num, vector<vector<int>> groups, double * &coverages)
{
	...
	for ( int i = 0; i<groups_num; i++)
	{
		...
		if(groups[i].size() == 0)//If the i-th group contains only 1 line segment, the span is 0
		{
			coverages[i] = 0;
		}
		else
...
//================================clustering==========================================================
//Clustering
//Find the sum of squared differences between each row i of points and each row j of initializations, with each row having nDims dimensions
...
/**
Input:
points: The set of points to be mean-shifted, with a total of nPoints points, each point having nDims dimensions, a one-dimensional array
initPoints: Mean-shift initialization position, the position to start searching in nxd space, with a total of initLength points, each point having nDims dimensions
sigma = 1
window_size: window parameter = distance_tolerance or window parameter = distance_tolerance/2
accuracy_tolerance: Convergence tolerance 1e-6
iter_times: 50 iterations
Output:
Converged position, the number of positions is the same as the number of initial search positions, we update the result to initPoints, it is both an input parameter and an output parameter, saving memory
*/
...
//		cout<<initPoints[2*i]<<'\t'<<initPoints[2*i+1]<<endl;
    ...
    double radius2 = window_size *window_size;//Squared
    double tolerance = accuracy_tolerance;
    int iters, maxiters = iter_times;//Maximum number of iterations
   //Return an array of the same size as the initial search point set
    double * finals = (double*)malloc(nQuerries * nDims * sizeof(double));;//Pointer to the array of final positions
    memcpy(finals, initializations, nQuerries * nDims * sizeof(double));
	double * distances = (double*)malloc(nPoints*sizeof(double));
    //printf("meanShift: nPoints:%d \tnDims: %d \tnQuerries:%d \n",nPoints,nDims,nQuerries);//print
    for (int loop = 0; loop < nQuerries; ++loop)
    {
        ...
        while (iters < maxiters)
        {
            ...
            for (int i = 0; i < nPoints; ++i)//Find all points that fall within the search circle
            {
                distances[i] = squaredDifference(nDims, points, i, initializations, loop);//Calculate the squared distance
                if (distances[i] <= radius2)//Within the circle with center at the loop-th search center and radius sqrt(radius2)
                {
                    flag = true;
            ...
            for (int j = 0; j < nDims; ++j)
				finals[loop*nDims+j] = 0;//Set the vector of the final position point to 0 for the loop-th search center
            for (int i = 0; i < nPoints; ++i)
                if (distances[i] <= radius2)
                {
                    for (int j = 0; j < nDims; ++j)//Each internal point vector is accumulated with a certain weight
						finals[loop*nDims+j] += exp(-distances[i] / sigma2) * points[i*nDims+j];
                }
            for (int j = 0; j < nDims; ++j)//Normalize the weight
				finals[loop*nDims+j] /= denominator;
            if (sqrt(squaredDifference(nDims, finals, loop, initializations, loop)) < tolerance)//If the difference between two consecutive iterations is within the tolerance, then it is considered to have converged, and there is no need to continue iterating
                break;
            iters = iters + 1;
            for (int j = 0; j < nDims; ++j)//Update the search center for iteration
...
/***
Input:
points, the set of points to be clustered, a one-dimensional array, nPoints points, each point with dimension nDims
distance_threshold determines the distance threshold for clustering
Output outPoints
Clustered point set nOutPoints x nDims
This function must be called after use, and the function internally allocates nOutPoints double arrays. After external use, remember to free(outPoints).
*/
...
{ 
    ...
    memset(labeled, 0, nPoints * sizeof(char));//Initialize the bool label to 0
	if(nPoints == 1)
	{
		        ...
		        for (int j = i+1; j < nPoints; ++j)
		        {
		            if (!labeled[j])
		            {
		                ...
		                if (d <= threshold2)
		                {
		                    ...
		                    labeled[j] = 1;
							if(counts[centers.size() - 1] >= number_control)//Control the number of clusters, prevent the mean-shift from drifting too far. 20 for circular center clustering, 10 for radial clustering
								break;
		                }
		            }
		        }
...
//		cout<<endl;
    ...
    centers.resize(0);
    counts.resize(0);
//vector<double>().swap(centers);//Release and recycle vector memory
//	vector<int>().swap(counts);
}

//Mean-shift clustering algorithm
//Three steps: first, select the initial iteration points; second, mean-shift; third, remove duplicate points to obtain the cluster centers
//Obtain the candidate circular centers (xi, yi)
//Input:
//points, one-dimensional point data, length points_num x 2
//distance_tolerance, data point clustering radius
//Output:
//centers is a one-dimensional double array, size centers_num x 2
//Correct return value is 1, error is 0. For example, if points is empty
//Note! centers is memory allocated internally by the function, used to return the cluster centers of centers_num points. After use, remember to free(centers)!
int  cluster2DPoints( double * points, int points_num, double distance_tolerance, double * & centers, int * centers_num)
{
	...
	xmax += xmax*0.02;//Avoid xdelta, ydelta being 0
	xmin -= xmin*0.02;
	ymax += ymax*0.02;
	ymin -= ymin*0.02;
	xdelta = (xmax-xmin);
	ydelta = (ymax-ymin);//There is a problem, if all data is the same size, this will be 0
	nbins_x = (int)ceil(xdelta/distance_tolerance);
	nbins_y = (int)ceil(ydelta/distance_tolerance);
	if(nbins_x <= 0 )
	{
		nbins_x = 1;//At least keep 1 bin
	...
	center_bins = (point2d1i *)calloc(nbins_y*nbins_x, sizeof(point2d1i));//(x, y, z), x is used to record sum(xi), y is used to record sum(yi), z is used to record the number of points falling in the grid
	memset(center_bins,0,sizeof(point2d1i)*nbins_y*nbins_x);
	if(center_bins == NULL)
		error("cluster2DPoints, not enough memory");
//cout<<"2D original data:"<<points_num<<endl;
	for ( i = 0; i< points_num; i++ )//Record the circular center to the grid, and increment the count of points falling in the corresponding grid by 1
	{
		addr = 2*i;
...
//		cout<<points[addr]<<'\t'<<points[addr+1]<<endl;
	...
	for ( y = 0; y<nbins_y; y++)//For non-zero voted grids, take the average of the points, and rewrite them to center_bins in order without consuming memory
		for ( x = 0; x<nbins_x; x++)
		{
			addr = y*nbins_x+x;
	...
	//Record the circular centers recorded in the linked list after partitioning to an array to facilitate use as initial points for mean-shift
	for ( i = 0; i<initCentersLength; i++ )//initCenters size is initCentersLength * 2
	{
		int addr = 2*i;
		initCenters[addr]   = center_bins[i].x;
		initCenters[addr+1] = center_bins[i].y;
	}
	free((void*)center_bins);//Quickly release this memory

//cout<<"2D mean-shift before initial iteration points:"<<endl;
//	for (int  i = 0; i<initCentersLength; i++)
//		cout<<initCenters[2*i]<<'\t'<<initCenters[2*i+1]<<endl;
	
	//The mean-shift result will be updated to initCenters
	meanShift(points,points_num,2,initCenters,initCentersLength,1,distance_tolerance,1e-6,50);//Iterate 20 times

//cout<<"2D mean-shift clustering centers:"<<endl;
//	for (int  i = 0; i<initCentersLength; i++)
//		cout<<initCenters[2*i]<<'\t'<<initCenters[2*i+1]<<endl;

	//Clustering
	//Note! centers_num is an int pointer, ++-- when used. (*centers_num).
	clusterByDistance(initCenters,initCentersLength,2,distance_tolerance/2,40,centers, centers_num);//This parameter needs to be adjusted

//cout<<"2D distance clustering, removing duplicate points:"<<endl;
//	for (int  i = 0; i<(*centers_num); i++)
//		cout<<centers[2*i]<<'\t'<<centers[2*i+1]<<endl;

	if((*centers_num) <= 0)//Can be omitted
	{
		return 0;  //Don't understand why, there are no points closest to the cluster center around the cluster center
		//system("pause");
		//error("cluster2DPoints,(*centers_num)<=0");
	}
	free(initCenters);
	//cout<<"2D number of clusters:"<<(*centers_num)<<endl;
	return 1;
}
...
//Obtain the candidate circular centers (xi, yi)
//Input:
//datas, one-dimensional point data, length datas_num x 1
//distance_tolerance, data point clustering radius
//Output:
//centers is a one-dimensional double array, size centers_num x 1
//Correct return value is 1, error is 0. For example, if points is empty
//Note! centers is memory allocated internally by the function, used to return the cluster centers of centers_num points. After use, remember to free(centers)!
int  cluster1DDatas( double * datas, int datas_num, double distance_tolerance, double * & centers, int * centers_num)
{
	...
	rmin = DBL_MAX;
	rmax = 0;
	for( i  = 0; i < datas_num; i++)//Copy the set r to an array
	{
		if(datas[i] < rmin)//Record the maximum and minimum values in this traversal
			rmin = datas[i];
		if(datas[i] > rmax)
			rmax = datas[i];
	}
	...
	rmax += rmin*0.02;//Avoid rmax - rmin = 0
	rmin -= rmin*0.02;
	rdelta = rmax - rmin;
	nbins_r = (int)ceil((rdelta)/distance_tolerance);
	if(nbins_r <= 0)//At least have one bin
		nbins_r = 1;
	center_bins = (point1d1i *)malloc(sizeof(point1d1i)*nbins_r);
	memset(center_bins,0,sizeof(point1d1i)*nbins_r);//Initialize to 0
//cout<<"1D original data:"<<datas_num<<endl;
	for( i = 0; i<datas_num; i++)//Vote for partitions
	{
//		cout<<datas[i]<<endl;
	...
	for( i = 0; i<nbins_r; i++)
	{
		if(center_bins[i].cnt > 0)//Count non-zero partitions, and take the average of each partition, rewrite to center_bins in order without consuming memory
		{
	...
	//Record the circular centers recorded in the linked list after partitioning to an array to facilitate use as initial points for mean-shift
	for ( i = 0; i<init_r_length; i++ )//initCenters size is init_r_length x 1
	{
		initCenters[i] = center_bins[i].data;
	}
	free(center_bins);//Quickly release this memory

//cout<<"1D mean-shift before initial iteration points:"<<endl;
//	for (int  i = 0; i<init_r_length; i++)
//		cout<<initCenters[i]<<'\t';
//	cout<<endl;

	//At this point, the initial iteration points for mean-shift have been obtained, which is a one-dimensional double array with length init_r_length
	meanShift(datas, datas_num, 1, initCenters, init_r_length, 1, distance_tolerance, 1e-6, 20);//Iterate 20 times

//cout<<"1D mean-shift clustering centers:"<<endl;
//	for (int  i = 0; i<init_r_length; i++)
//		cout<<initCenters[i]<<'\t';
//	cout<<endl;
	...
	//Clustering
	//Note! centers_num is an int pointer, ++-- when used. (*centers_num).
	clusterByDistance(initCenters, init_r_length, 1, distance_tolerance/2, 40, centers, centers_num);//Control parameter 40, a maximum of 40 points merged into 1 point
	
//cout<<"1D distance clustering, removing duplicate points:"<<endl;
//	for (int  i = 0; i<(*centers_num); i++)
//		cout<<centers[i]<<'\t';
//	cout<<endl;
    ...
    free(initCenters);
//cout<<"1D number of clusters:"<<(*centers_num)<<endl;
	return 1;
}

//================================Generate Ellipse Candidates=========================================
//Matching group pairs, group pair index parameters, ellipse parameters
...
//Calculate the gradient, return the modulus and angle, and directly suppress pixels with too small modulus, setting them to NOTDEF
//mod, angles are passed by value, they are second-level pointers
...
void calculateGradient2( double * img_in, unsigned int imgx, unsigned int imgy, image_double * angles)
{
	...
	threshold = 2*sqrt(sum/(imgx*imgy));//Automatic threshold
                ...
                int aperture_size )
{
  ...
  // cv::Mat dx = cv::cvarrToMat(dxarr);
  // cv::Mat dy = cv::cvarrToMat(dyarr);
...
void cvCanny3cpp(	const Mat& srcarr, Mat& dstarr,
  Mat& dxarr, Mat& dyarr,
          int aperture_size )
{
//cv::Ptr<cv::Mat> dx, dy;
...
cv::Mat dst = dstarr;//.clone();


cv::Mat dx = dxarr;//.clone();
cv::Mat dy = dyarr;//.clone();
...
CV_MAT_TYPE( dy.type()  ) != CV_16SC1 ){
  // CV_Error( CV_StsUnsupportedFormat, "" );
  error("here should be cv_stsunsupportedformat error");
}

if( src.size()!=dst.size()){
  // CV_Error( CV_StsUnmatchedSizes, "" );
  error("here should be cv_stsunmatchedsizes error "+cv::format("src: %d %d dst: %d %d",src.size().width,src.size().height,dst.size().width,dst.size().height));
}

aperture_size &= INT_MAX;
if( (aperture_size & 1) == 0 || aperture_size < 3 || aperture_size > 7 ){
  // CV_Error( CV_StsBadFlag, "" );
  error("here should be cv_stsbadflag error");
}
...
//aperture_size = -1; //SCHARR
cv::Sobel( src, dx,dx.type(), 1, 0, aperture_size );
cv::Sobel( src, dy, dx.type(),0, 1, aperture_size );
...
void Canny3cpp(	const Mat &image,  Mat& _edges,
				Mat& _sobel_x, Mat& _sobel_y,
                int apertureSize, bool L2gradient )
{
    Mat src = image.clone();
    _edges.create(src.size(), CV_8U);
	_sobel_x.create(src.size(), CV_16SC1);
	_sobel_y.create(src.size(), CV_16SC1);


    Mat c_src = src;
    Mat c_dst = _edges;//.clone();
	Mat c_dx = _sobel_x;//.clone();
	Mat c_dy = _sobel_y;//.clone();

 
    cvCanny3cpp(	c_src, c_dst, 
				c_dx, c_dy,
				apertureSize + (L2gradient ? Constants::L2gradient : 0));
};
//*/
...
void calculateGradient3( double * img_in, unsigned int imgx, unsigned int imgy, image_double * angles)
{
	...
	//canny
   Canny3cpp(gray,edge,DX,DY,3,false);
   for ( y = 0; y<imgy; y++)
   {
...
int ellipse2Param(double *p,double param[])
{
 ...
 //     else Rv=-pow(-Rv,0.5);
	  if (Ru <= 0 || Rv <= 0)//Will there be a situation where Ru < Rv? Adjust if necessary.
		  return 0;
	  Ru = sqrt(Ru);
	  Rv = sqrt(Rv);
      param[0]=uCentre;param[1]=vCentre;
      param[2]=Ru;param[3]=Rv;param[4]=thetarad;
	  //Swap the long and short axes to make the third parameter the long axis and the fourth parameter the short axis
	  if(Ru < Rv )
	  {
		  param[2] = Rv;
		  param[3] = Ru;
		  if(thetarad < 0)//Swap the long and short axes to ensure the long axis tilt angle is within -pi/2 ~ pi/2, ensuring uniqueness
			  param[4] += M_1_2_PI;
		  else
			  param[4] -= M_1_2_PI;
		  if(thetarad < - M_1_2_PI)//Long axis tilt angle limited to -pi/2 ~ pi/2, ensuring uniqueness
			  param[4] += M_PI;
...
//input : (xi,yi)
//Output: x0, y0, a, b, phi, ellipara needs to be allocated memory first
...
int fitEllipse(point2d* dataxy, int datanum, double* ellipara)
{
	...
	//Note that S is a symmetric matrix, so transposing it is equal to itself, becoming column-major order, S can remain unchanged
	dggev_(&JOBVL,&JOBVR,&fitN,S,&fitN,C,&fitN,alphar,alphai,beta,vl,&fitN,vr,&fitN,fitWork,&workLen,&info,0,0);
	if(info == 0)
	{
		int index = -1;
		for ( int i = 0; i<6; i++)
			if( (alphar[i]>=-(2.2204460492503131e-014)) && (alphai[i] == 0) && (beta[i] != 0)) // 100*DBL_EPSILON, eigenvalue = (alphar + i*alphai)/beta
				index = i;//The i-th column of vr corresponds to the eigenvector, which is the fitting parameter
		if(index == -1)//Relax the constraint on the real part > 0 to > -0.005
		{
			double temp = -0.005;//This parameter is very important
			for ( int i = 0; i<6; i++)
		...
		if(index != -1)
		{
			//Borrow beta to pass the parameter
		    //beta[0] = vr[6*0+index];
		    //beta[1] = vr[6*1+index];
		    //beta[2] = vr[6*2+index];
		    //beta[3] = vr[6*3+index];
...
//Input: dataxy is the data point (xi, yi), with a total of datanum points
//Output: Fitting matrix S. Note: S needs to be allocated memory first, double S[36].
...
//S_out needs to be allocated memory first
...
//Input: S matrix, 6 x 6 = 36
//Output: (A, B, C, D, E, F) and A > 0, ellicoeff needs to be allocated memory first. When converting to (x0, y0, a, b, phi), use
...
int fitEllipse2(double * S, double* ellicoeff)
{
	...
	if(info == 0)
	{
		...
		{
			//Borrow beta to pass the parameter
	        if(vr[6*index+0] < 0)//Note column-major order
			{
...
//Input: e1 = (x1, y1, a1, b1, phi1), e2 = (x2, y2, a2, b2, phi2)
...
inline bool regionLimitation( point2d point_g1s, point2d g1s_ls_dir, point2d point_g1e, point2d g1e_ls_dir, point2d point_g2s, point2d g2s_ls_dir, point2d point_g2e, point2d g2e_ls_dir, double polarity, double region_limitation_dis_tolerance)
{
	...
	point2d test_vec1,test_vec2,test_vec3; //The vector pointing to the center of the arc and the test vector
	//The vector formed by pend - pstart of the group is the arc direction vector gim_arc_dir
...
//Input
//lsd algorithm detects line_num line segments, return returns line_num line segments, which is a one-dimensional double array lines of length 8 * n, each group of 8 represents x1, y1, x2, y2, dx, dy, width, polarity
//groups: Line segment grouping, each group stores the indices of the line segments in the order of geometric distribution clockwise or counterclockwise. The line segment indices range from 0 ~ line_num-1. Note that this is a pointer, so (*group) is used.
//first_group_ind, second_group_ind are the group pairing indices. When extracting salient hypotheses, second_group_ind = -1, fit_matrix2 = NULL.
//fit_matrix1, fit_matrix2 are the corresponding fitting matrices of the group pairs.
//angles is the edge pixel image + gradient direction. NOTDEF if there are no edge pixels.
//group_inliers_num: An array that records the number of supporting inliers for each group, updated in real-time, initially 0
//distance_tolerance:
//Output
//Fitting matrix S
//ellipara
...
{
	double S[36]; //Only perform fitting for one group with a large coverage
	...
	if( fit_matrix2 == NULL || second_group_ind == -1)//Perform fitting for the group pair, S = fit_matrix1 + fit_matrix2
	{
		for ( int i  = 0; i < 36; i++)
			S[i] = fit_matrix1[i];
	}
	...
	{
		addFitMatrix(fit_matrix1,fit_matrix2,S);//Fitting failed
	}
	info = fitEllipse2(S, Coefficients);// ax^2 + bxy + cy^2 + dx + ey + f = 0, a > 0
	if ( info == 0 )//Fitting failed
	{
		ellipara = NULL;
		return FALSE;
	}
	...
	//The first group in the group pair first performs inlier criterion verification and updates the number of supporting inliers for the group
	for ( unsigned int i = 0; i<(*groups)[first_group_ind].size(); i++)
	{
		addr = (*groups)[first_group_ind][i] * 8; //The i-th line segment index in the first group * 8
		rec.x1 = lines[addr];
		rec.y1 = lines[addr+1];
		rec.x2 = lines[addr+2];
		rec.y2 = lines[addr+3];
		...
		//Record the length of the line to the array line_length[i]
		rec_support_cnt = rec_inliers_cnt = 0;//Clear is very important
		if ( lines[addr+7] == 1) //Same polarity
		{
			for(iter = ri_ini(&rec);!ri_end(iter);ri_inc(iter))//Line segment 1
			{
				//The bounding box may be out of bounds
				if(iter->x >= 0 && iter->y >= 0 && iter->x < angles->xsize && iter->y < angles->ysize)
				{
					temp  = angles->data[iter->y*angles->xsize+iter->x] ;//Gradient direction of the inlier
					if(temp!= NOTDEF )
					{
						...
						point_normal = atan2(-point_normaly,-point_normalx); //Normal direction of the edge pixel, pointing inward to the ellipse
						rec_inliers_cnt++;
						if(angle_diff(point_normal,temp) <= M_1_8_PI ) //Within ±22.5° and || d - r || < 3 dis_t
						{
							rec_support_cnt++;
							pixel_temp.x = iter->x; pixel_temp.y = iter->y;
							first_group_inliers.push_back(pixel_temp);//Add the inliers corresponding to the line segment
						}
					} 
				}
			}
			...
			for(iter = ri_ini(&rec);!ri_end(iter);ri_inc(iter))//Line segment 1
			{
				...
				if(iter->x >= 0 && iter->y >= 0 && iter->x < angles->xsize && iter->y < angles->ysize)
				{
					...
					if(temp!= NOTDEF )
					{
						...
						point_normalx = Coefficients[0]*iter->x + (Coefficients[1]*iter->y + Coefficients[3])/2;
						point_normaly = Coefficients[2]*iter->y + (Coefficients[1]*iter->x + Coefficients[4])/2;
						point_normal = atan2(point_normaly,point_normalx); //Normal direction of the edge pixel, pointing outward to the ellipse
						rec_inliers_cnt++;
						if(angle_diff(point_normal,temp) <= M_1_8_PI ) //Within ±22.5° and || d - r || < 3 dis_t
						{
							rec_support_cnt++;
		...
		{
			flag1 = FALSE; //flag1 is initialized to TRUE. If any line segment in the group does not meet the requirement, set it directly to false. Inlier criterion verification fails
			break;
		}
	}
	if ( flag1 == TRUE && first_group_inliers.size() >= 0.8*group_inliers_num[first_group_ind] )//Close to the maximum number of inliers found, pass the verification
	{
		if( first_group_inliers.size() >= group_inliers_num[first_group_ind])//Update the maximum number of inliers found for the group
			group_inliers_num[first_group_ind] =  first_group_inliers.size();
	}
	else 
		flag1 = FALSE;
	//The first group completes verification
	if ( second_group_ind == -1 || fit_matrix2 == NULL)//Only perform fitting for one group with a large coverage
	{
		ellipara->x = param[0];//Because a significant ellipse must be returned no matter what
	    ellipara->y = param[1];
	    ellipara->a = param[2];
	    ellipara->b = param[3];
	    ellipara->phi = param[4];
		if ( flag1 == TRUE)//Improve the quality by refitting with inliers
		{
			...
			free(dataxy); //Release memory
			if ( info == 1  && isEllipseEqual(param2,param,3*distance_tolerance,0.1,0.1,0.1,0.9) )
			{
				ellipara->x = param2[0];//Update the ellipse, improve the quality
			    ellipara->y = param2[1];
			    ellipara->a = param2[2];
			    ellipara->b = param2[3];
		...
		return TRUE;//For extraction with only one group, return directly at this point
	}
	//Next, the second group in the group pair performs inlier criterion verification and updates the number of supporting inliers for the group
	if (flag1 == FALSE)//In the group pair operation, if the first group does not meet the inlier requirement, return false directly
		return FALSE;
	for ( unsigned int i = 0; i<(*groups)[second_group_ind].size(); i++)
	{
		addr = (*groups)[second_group_ind][i] * 8; //The i-th line segment index in the first group * 8
		rec.x1 = lines[addr];
		rec.y1 = lines[addr+1];
		rec.x2 = lines[addr+2];
		rec.y2 = lines[addr+3];
				...
				if(iter->x >= 0 && iter->y >= 0 && iter->x < angles->xsize && iter->y < angles->ysize)
				{
					...
					if(temp!= NOTDEF )
					{
						...
						if(angle_diff(point_normal,temp) <= M_1_8_PI ) //Within ±22.5° and || d - r || < 3 dis_t
						{
							rec_support_cnt++;
							pixel_temp.x = iter->x; pixel_temp.y = iter->y;
							second_group_inliers.push_back(pixel_temp);//Add the inliers corresponding to the line segment
						}
					} 
				}
			}
		...
		if( !(rec_support_cnt > 0 && ( rec_support_cnt >= 0.8*lines[addr+6] || rec_support_cnt*1.0/rec_inliers_cnt >= 0.6) ) )
		{
			flag2 = FALSE; //flag1 is initialized to TRUE. If any line segment in the group does not meet the requirement, set it directly to false. Inlier criterion verification fails
			break;
		}
	}
	if ( flag2 == TRUE && second_group_inliers.size() >= 0.8*group_inliers_num[second_group_ind] )//Close to the maximum number of inliers found, pass the verification
	{
		if(second_group_inliers.size() >= group_inliers_num[second_group_ind])//Update the maximum number of inliers found for the group
			group_inliers_num[second_group_ind] = second_group_inliers.size();
	}
	else 
		flag2 = FALSE;
	//The second group completes verification
	if ( flag1 == TRUE && flag2 == TRUE)
	{
		...
		for ( unsigned int i = 0; i<second_group_inliers.size(); i++)//Be careful with the index range when connecting two arrays
		{
...
//		drawEdge(img,dataxy,(first_group_inliers.size() + second_group_inliers.size()));
		...
		free(dataxy); //Release memory
		//Relax the parameters for small long and short axis ellipses
		if ( param[2] <= 50 )
...
//Input
//lsd algorithm detects line_num line segments, return returns line_num line segments, which is a one-dimensional double array lines of length 8 * n, each group of 8 represents
// x1, y1, x2, y2, dx, dy, length, polarity
//groups: Line segment grouping, each group stores the indices of the line segments in the order of geometric distribution clockwise or counterclockwise. The line segment indices range from 0 ~ line_num-1.
//coverages: The angle coverage range of each group is 0 ~ 2pi. If a group contains only 1 line segment, the coverage angle is 0. The array length is equal to the number of groups.
//angles stores the gradient direction of the edge pixels, NOTDEF if there are no edge pixels.
//Return value PairedGroupList* list returns the initial ellipse set array, with length list->length. 
//Note: This memory is allocated internally by the function. After using the function, remember to release the memory. Call the function freePairedSegmentList() to release the memory.
...
PairGroupList * getValidInitialEllipseSet( double * lines, int line_num, vector<vector<int>> * groups, double * coverages, image_double angles, double distance_tolerance, int specified_polarity)
{
	...
	//int* lineInliersIndex = (int*)malloc(sizeof(int)*line_num);//If the i-th line segment finds inliers, then record its index as j = length(supportInliers), i.e., supportInliers.at(j) stores the inliers of the line segment. If no inliers are found, the initial value is -1.
    //vector<vector<point2d>> supportInliers;//Stores the corresponding inliers of the line segments
	//memset(lineInliersIndex,-1,sizeof(int)*line_num);//This can be done in practice, for integers it can be initialized to 0, -1. For floating-point numbers, it can only be initialized to 0.
	...
	double * fitMatrixes = (double*)malloc(sizeof(double)*groupsNum*36);//Define the fitting matrix S_{6 x 6}. Each group has one fitting matrix
	unsigned int * supportInliersNum = (unsigned int*)malloc(sizeof(int)*groupsNum);//Used to store the maximum number of supporting inliers found for each group
	memset(fitMatrixes,0,sizeof(double)*groupsNum*36);
	memset(supportInliersNum, 0, sizeof(unsigned int)*groupsNum);//Initialize to 0.
	...
	//Instantiate the fitting matrix Si
	point2d * dataxy = (point2d*)malloc(sizeof(point2d)*line_num*2);//Allocate sufficient memory, line_num line segments, 2 * line_num endpoints in total
	for ( i = 0; i<groupsNum; i++)
	{
		cnt_temp = 0;//Must clear to 0
		for ( j = 0; j<(*groups)[i].size(); j++)
		{
			//Each line segment has 2 endpoints
	...
	head = tail = NULL;//Store the initial ellipse set to the linked list
	//selection of salient elliptic hypothesis
	for ( i = 0; i<groupsNum; i++)
	{
		if(coverages[i] >= M_4_9_PI )//When the group's coverage angle >= 4pi/9 = 80°, we consider it to have high significance and can be directly extracted
		{
			//Add polarity judgment, only extract ellipses with the specified polarity
			if (specified_polarity == 0 || (lines[(*groups)[i][0]*8+7] == specified_polarity))
			{
				//Highly significant initial ellipses are always returned, so there is no need to judge again
				...
				{
					continue;
					error("getValidInitialEllipseSet, selection of salient ellipses failed!");//Does this situation occur? It occurred when running 54.jpg
				}
			   ...
			   if (specified_polarity == 0 || (lines[(*groups)[i][0]*8+7] == specified_polarity))
			    {
					...
					{
						ind_start = (*groups)[i][0];//The index of the first line segment in the i-th group
						ind_end   = (*groups)[i][(*groups)[i].size()-1];//The index of the last line segment in the i-th group
						...
						ind_start = (*groups)[j][0];//The index of the first line segment in the j-th group
						ind_end   = (*groups)[j][(*groups)[j].size()-1];//The index of the last line segment in the j-th group
						...
						polarity = lines[ind_start*8+7]; //Polarity of the i and j groups
						if(regionLimitation(pointG1s,g1s_ls_dir,pointG1e,g1e_ls_dir,pointG2s,g2s_ls_dir,pointG2e,g2e_ls_dir,polarity,-3*distance_tolerance))//Both are within each other's linear region
						{
							//if ( i == 2)
							//	drawPairGroup(img,lines,(*groups),i,j);

							if(calcEllipseParametersAndValidate(lines,line_num,groups,i,j,(fitMatrixes+i*36),(fitMatrixes+j*36),angles,distance_tolerance,supportInliersNum,&ellipara))//Quadratic general equation linear solution, inlier support ratio of line segments
							{
								PairGroupNode * node = (PairGroupNode*)malloc(sizeof(PairGroupNode));
								node->center.x = ellipara.x;
								node->center.y = ellipara.y;
	...
	if(pairlength > 0)
	{
		...
		for( int i = 0; i<pairGroupList->length; i++)
		{
			...
			pairGroupList->pairGroup[i].pairGroupInd.x = p->pairGroupInd.x;//Record the group pair (i, j), the valid elliptical parameters are generated by the matching pair formed by the i-th group and the j-th group in groups
	...
	//free(lineInliersIndex);//Release the index of line segment inliers
	free(supportInliersNum);//Release the array storing the number of supporting inliers for each group
	free(fitMatrixes);//Release the array storing the fitting matrix for each group
	return pairGroupList;
}
...
{
	if( pairGroupList->length <= 0 )//Detection, at least one sample is needed to generate a candidate
	{
	...
	double * centers;
	int center_num; //Number of clusters of ellipse centers (xi, yi)
	double * phis;
	int phi_num;    //For each ellipse center (xi, yi), the number of clusters of the tilt angle phi
	double * axises;
	int axis_num;   //For each ellipse center and tilt angle (xi, yi, phi), the number of clusters of the long and short semi-axes (a, b)
	...
	point2i * bufferIndexes = (point2i *)calloc(pairGroupList->length,sizeof(point2i));//point[i].x records the starting index position of the i-th classification in bufferXX, point[i].y records the length of the i-th classification in bufferXX
	double  * buffer2AB = (double*)calloc(pairGroupList->length*2,sizeof(double));
	point2i * buffer2Indexes = (point2i *)calloc(pairGroupList->length,sizeof(point2i));//point[i].x records the starting index position of the i-th classification in bufferXX, point[i].y records the length of the i-th classification in bufferXX
	...
	(*candidates_num) = 0; //Number of candidate ellipses, initialize to 0, very important
	//copy
	for ( int i = 0; i<pairGroupList->length; i++)
	{
		addr = 2*i;
	...
	//classification, find the cluster center to which each point belongs
	for ( int i = 0; i<pairGroupList->length; i++)
	{
		dis_min = DBL_MAX;
		ind = -1;
		...
		buffer_temp[i] = ind; //Borrow buffer2 to record the i-th initial ellipse corresponding to the ind-th ellipse cluster center
	}
	//Store the classification results in bufferXY, bufferPhi, bufferAB in order, and bufferIndexes[i] stores the starting index position and length of the i-th cluster center
	memset(bufferIndexes,0,sizeof(point2i)*pairGroupList->length);
	ind = 0;//Clear to zero, the starting position of the sample point, the index position is ind * 2, the starting position of the partition, the base index
	for ( int i = 0; i<center_num; i++)
	{
		...
		for ( int j = 0; j<pairGroupList->length; j++)
		{
			if ( buffer_temp[j] == i)
			{
				addr = ind*2;//Note that the long and short semi-axes are stored in pairs, need to x 2
				...
				bufferIndexes[i].y++;//Increment the number of points around the i-th cluster center by 1
			}
		}
		if(bufferIndexes[i].y == 0)//No points around the cluster center
		{
	...
	//Perform tilt angle clustering for the points around each ellipse center
	//The index range of the i-th ellipse cluster center is bufferIndexs[i].x ~ (bufferIndexs[i].x + bufferIndexs[i].y - 1)
	for ( int i = 0; i<center_num; i++)
	{
		

		double * phi_pointer_temp = bufferPhi+bufferIndexes[i].x;//Tilt angle pointer
		double * ab_pointer_temp = bufferAB+bufferIndexes[i].x*2;//Long and short semi-axes pointer, note x 2
		info = cluster1DDatas(phi_pointer_temp, bufferIndexes[i].y, 0.0873, phis, &phi_num);//Cluster phi, error tolerance 0.0873, 5° error
		if (info == 0) //Don't understand why, the cluster center centers[i] may not have the closest point around it, the number of points bufferIndexs[i].y = 0
		{ 
		...
		//Store the classification results in buffer2AB in order, and buffer2Indexes[j].x corresponds to the starting point of the j-th phi cluster center, buffer2Indexes[j].y corresponds to the number (length)
		memset(buffer2Indexes,0,sizeof(point2i)*bufferIndexes[i].y);
		ind = 0;
		for ( int j = 0; j<phi_num; j++)
		{
			buffer2Indexes[j].x = ind;//Starting point
			for ( int k = 0; k<bufferIndexes[i].y; k++)
			{
				if ( buffer_temp[k] == j)
				{
					...
					buffer2Indexes[j].y++;//Increment the length by 1
				}
			}
		...
		for ( int j = 0; j<phi_num; j++ )
		{
			double * ab_pointer_temp2 = buffer2AB+buffer2Indexes[j].x*2; //Long and short semi-axes pointer, note x 2
			info = cluster2DPoints(ab_pointer_temp2, buffer2Indexes[j].y, distance_tolerance, axises, &axis_num);
			if (info == 0) //Don't understand why, the cluster center phi_j may not have the closest point around it, the number of points buffer2Indexes[j].y = 0
			{   
			...
			//Rewrite the candidate ellipses to bufferXY, bufferPhi, bufferAB, increment the number of candidate ellipses (*candidates_num)++
			...
			free(axises);//cluster2DPoints strictly requires that after using the function, the internally allocated memory must be released. Free axises after use.
		}
		free(phis);//cluster1DDatas strictly requires that after using the function, the internally allocated memory must be released. Free phis after use.
	}
	free(centers);//cluster2DPoints strictly requires that after using the function, the internally allocated memory must be released. Free centers after use.
	//Release part of the memory allocated at the beginning of the function
	free(buffer_temp); //There is a problem with this release
	free(buffer2Indexes);
	free(buffer2AB);
	free(bufferIndexes);
	...
	//Release the memory allocated at the beginning of the function
	free(bufferAB);
	free(bufferPhi);
	free(bufferXY);
	if((*candidates_num)<= 0)
...
/**
Input:
prhs[0]: Input grayscale image, single-channel, size imgy x imgx
prhs[1]: Edge extraction selection, 1 canny; 2 sobel
prhs[2]: Detected ellipse polarity
Output:
plhs[0]: Candidate ellipse combination (xi, yi, ai, bi, phi_i)', 5 x m
plhs[1]: Edge image, size imgy x imgx, binary image, 0 or 255
plhs[2]: Gradient vector matrix of edge pixels, size 2 x edgepix_n, (cos(theta_rad), sin(theta_rad))'...
plhs[3]: Line segment image, size imgy x imgx
*/
/*
Compile:
...
// 	int imgy,imgx;
// 	int edge_process_select = (int)mxGetScalar(prhs[1]);//Edge extraction selection, 1 canny; 2 sobel
// 	int specified_polarity  = (int)mxGetScalar(prhs[2]);//1, specify to detect positive polarity ellipses; -1 specify negative polarity; 0 detect both polarities
// 	imgy = (int)mxGetM(prhs[0]);
// 	imgx = (int)mxGetN(prhs[0]);
// 	double *data=(double*)malloc(imgy*imgx*sizeof(double));//Store the image data from the input matrix into a one-dimensional array
...
// 	free(reg); //Release memory
// 	calcuGroupCoverage(out,n,groups,coverages);//Calculate the coverage angle of each group
...
// 	 double * candidates; //Candidate ellipses
// 	 double * candidates_out;//Output candidate ellipse pointer
// 	 int  candidates_num = 0;//Number of candidate ellipses
...
// 		//Copy the candidate ellipse combination (xi, yi, ai, bi, phi_i)', 5 x candidates_num, to the matrix candidates_out
...
// 	 }
// 	 uchar *edgeimg_out;
// 	 unsigned long edge_pixels_total_num = 0;//Total edge pixels
// 	 double *gradient_vec_out;
// 	 plhs[1] = mxCreateNumericMatrix(imgy,imgx,mxUINT8_CLASS,mxREAL);
// 	 edgeimg_out = (uchar*)mxGetData(plhs[1]);
// 	 //Copy the edge image to the matrix edgeimg_out
// 	 //Store the gradient vector to the matrix gradient_vec_out
...
// 			 else
// 			 {
// 				 edgeimg_out[c*imgy+r] = 255;//Set to white for edge pixels
// 				 //------------------------------------------------
// 				 edge_pixels_total_num++;
// 			 }
// 		 }
// 	 printf("edge pixel number: %i\n",edge_pixels_total_num);
// 	//Allocate edge_pixels_total_num x 2 to store the gradient vector of each edge pixel, column-major order, following Matlab's convention
...
// 		 }
// 	 //---------------------------------------------------------------------
// 	//Output the line segment detection image
// 	if(nlhs == 4)
// 	{
...
// 	}
// 	//---------------------------------------------------------------------
// 	//This releases the memory used by the program to generate candidates
// 	free(data);
// 	free(coverages);
// 	free(out);
// 	free_image_double(angles);
...
/**
 * same as above, but without mex classes
Input:
: Input grayscale image, single-channel, size imgy x imgx
: Edge extraction selection, 1 canny; 2 sobel
: Detected ellipse polarity
Output:
: Candidate ellipse combination (xi, yi, ai, bi, phi_i)', 5 x m
: Edge image, size imgy x imgx, binary image, 0 or 255
: Gradient vector matrix of edge pixels, size 2 x edgepix_n, (cos(theta_rad), sin(theta_rad))'...
: Line segment image, size imgy x imgx
*/



// Function to encapsulate the logic from mexFunction
// Returns 0 on success, -1 on failure
int processImageForEllipses(const std::string& imagePath, int edge_process_select, int specified_polarity,
                            std::vector<double>& candidate_ellipses, // Output parameter
                            cv::Mat& edge_image,                     // Output parameter
                            std::vector<double>& gradient_vectors,   // Output parameter
                            cv::Mat* ls_image_out = nullptr)          // Optional output parameter
{
    // Read the input image using OpenCV
    cv::Mat input_img_mat = cv::imread(imagePath, cv::IMREAD_GRAYSCALE);

    if (input_img_mat.empty()) {
        std::cerr << "Error: Could not read image from " << imagePath << std::endl;
        return -1; // Indicate failure
    }

    int imgy = input_img_mat.rows;
    int imgx = input_img_mat.cols;

    // Allocate memory and copy image data from OpenCV Mat (uchar, row-major)
    // to double* data (likely needed as row-major by mylsd and calculateGradient)
    // The original MEX code copy loop suggests converting MATLAB's col-major uchar
    // to C++'s row-major double. Let's adapt this for OpenCV's row-major uchar.
    ...
    // Copy and convert uchar to double
    for (int r = 0; r < imgy; ++r) {
        for (int c = 0; c < imgx; ++c) {
            // OpenCV Mat data is row-major: input_img_mat.at<uchar>(r, c)
            // Original MEX copied inputimg[r+c*imgy] (Matlab col-major) to data[c+r*imgx] (C++ row-major)
            // Replicate the C++ row-major indexing for the 'data' array
            data[r * imgx + c] = static_cast<double>(input_img_mat.at<uchar>(r, c));
        }
    }

    int n; // Number of line segments
    std::vector<std::vector<int>> groups; // Grouped line segment indices
    double *coverages = NULL; // Group coverage angles (will be allocated by calcuGroupCoverage)
    int *reg = NULL; // Region data from LSD (will be allocated by mylsd)
    int reg_x, reg_y;

    // Perform Line Segment Detection
    double* out = mylsd(&n, data, imgx, imgy, &reg, &reg_x, &reg_y);
    if (!out || !reg) {
         std::cerr << "Error: mylsd failed." << std::endl;
         free(data);
         if (reg) free(reg); // Ensure reg is freed if mylsd allocated it before failing 'out'
         if (out) free(out);
         return -1;
    }
    ...
    free(reg); // Release region memory after grouping

    // Calculate group coverage
    calcuGroupCoverage(out, n, groups, coverages); // coverages is allocated inside or before this call

    std::cout << "The number of output arc-support line segments: " << n << std::endl;
    std::cout << "The number of arc-support groups: " << groups.size() << std::endl;

    // Calculate gradient
    image_double angles = NULL;
    if (edge_process_select == 1) {
        // Canny (as per original comment "version 3 canny")
        calculateGradient3(data, imgx, imgy, &angles);
    } else {
        // Sobel (as per original comment "version2, sobel")
        calculateGradient2(data, imgx, imgy, &angles);
    }

    if (!angles || !angles->data) {
         std::cerr << "Error: Gradient calculation failed." << std::endl;
         free(data);
         free(out);
         if (coverages) free(coverages);
         if (angles) free_image_double(angles);
         return -1;
    }


    double distance_tolerance = std::max(2.0, 0.005 * std::min(angles->xsize, angles->ysize)); // 0.005%*min(xsize,ysize)
    double *candidates = NULL; // Candidate ellipses (will be allocated by generateEllipseCandidates)
    int candidates_num = 0;

    // Get valid initial ellipse sets
    PairGroupList *pairGroupList = getValidInitialEllipseSet(out, n, &groups, coverages, angles, distance_tolerance, specified_polarity);

    if (pairGroupList != NULL) {
        std::cout << "The number of initial ellipses: " << pairGroupList->length << std::endl;
        // Generate final ellipse candidates
        generateEllipseCandidates(pairGroupList, distance_tolerance, candidates, &candidates_num);
        std::cout << "The number of ellipse candidates: " << candidates_num << std::endl;

        // Copy candidate ellipses to output vector
        if (candidates && candidates_num > 0) {
             candidate_ellipses.resize(5 * candidates_num);
             memcpy(candidate_ellipses.data(), candidates, sizeof(double) * 5 * candidates_num);
        }

        freePairGroupList(pairGroupList);
        if (candidates) free(candidates); // Release memory allocated by generateEllipseCandidates

    } else {
        std::cout << "The number of initial ellipses: 0" << std::endl;
        // Output an empty or zeroed candidate list as in MEX
        candidate_ellipses.assign(5, 0.0);
        candidates_num = 0; // Explicitly set candidate count to 0
    }

    // Prepare edge image output
    edge_image.create(imgy, imgx, CV_8UC1);
    edge_image.setTo(0); // Initialize with black

    // Prepare gradient vector output
    ...
    for (int r = 0; r < imgy; ++r) {
        for (int c = 0; c < imgx; ++c) {
            ...
            if (angles->data[addr] != NOTDEF) {
                edge_image.at<uchar>(r, c) = 255; // Set to white for edge pixels
                edge_pixels_total_num++;
            }
        }
    }
    std::cout << "edge pixel number: " << edge_pixels_total_num << std::endl;

    // Store gradient vectors for edge pixels
    gradient_vectors.resize(2 * edge_pixels_total_num);
    unsigned long g_cnt = 0;
     for (int r = 0; r < imgy; ++r) {
        for (int c = 0; c < imgx; ++c) {
            unsigned long addr = r * imgx + c;
             if (angles->data[addr] != NOTDEF) {
                 gradient_vectors[g_cnt++] = cos(angles->data[addr]);
                 gradient_vectors[g_cnt++] = sin(angles->data[addr]);
             }
         }
     }

    // Generate optional line segment/ellipse image
    if (ls_image_out) {
        ls_image_out->create(imgy, imgx, CV_8UC1);
        ls_image_out->setTo(0); // Initialize with black

        // Draw lines
        // The 'out' format seems to be [x1, y1, x2, y2, ...], 8 doubles per segment
        for (int i = 0; i < n; ++i) {
            cv::Point2d p1(out[8 * i], out[8 * i + 1]);
            cv::Point2d p2(out[8 * i + 2], out[8 * i + 3]);
            cv::line(*ls_image_out, p1, p2, cv::Scalar(255), 1); // Draw white lines
        }

        // Draw ellipses
        // The 'candidate_ellipses' format is [x, y, a, b, phi] flattened (5*candidates_num total elements)
        if (candidates_num > 0) {
            for (int i = 0; i < candidates_num; ++i) {
                cv::Point center(static_cast<int>(candidate_ellipses[i * 5]), static_cast<int>(candidate_ellipses[i * 5 + 1]));
                cv::Size axes(static_cast<int>(candidate_ellipses[i * 5 + 2]), static_cast<int>(candidate_ellipses[i * 5 + 3]));
                double angle_deg = candidate_ellipses[i * 5 + 4] * 180.0 / M_PI;
                // Ensure angle is non-negative and within [0, 360) if required by cv::ellipse, though it usually handles it.
                // double angle_deg_normalized = fmod(angle_deg, 360.0); if (angle_deg_normalized < 0) angle_deg_normalized += 360;
                cv::ellipse(*ls_image_out, center, axes, angle_deg, 0, 360, cv::Scalar(255), 1); // Draw white ellipses
            }
        }
    }

    // Release memory allocated by malloc/custom functions
    free(data);
    if (coverages) free(coverages); // Check if coverages was allocated
    free(out);
    free_image_double(angles);
...
int main(int argc, char** argv) {
    if (argc < 4 || argc > 5) {
        std::cerr << "Usage: " << argv[0] << " <image_path> <edge_method> <polarity> [output_ls_image_path]" << std::endl;
        std::cerr << "  <edge_method>: 1 for Canny, 2 for Sobel" << std::endl;
        std::cerr << "  <polarity>: 1 for positive, -1 for negative, 0 for both" << std::endl;
        return EXIT_FAILURE;
    }

    std::string imagePath = argv[1];
    int edge_method = std::stoi(argv[2]);
    int polarity = std::stoi(argv[3]);
    std::string outputLsImagePath = (argc == 5) ? argv[4] : "";

    if (edge_method != 1 && edge_method != 2) {
        std::cerr << "Invalid edge method. Use 1 (Canny) or 2 (Sobel)." << std::endl;
        return EXIT_FAILURE;
    }
    if (polarity != -1 && polarity != 0 && polarity != 1) {
        std::cerr << "Invalid polarity. Use 1 (positive), -1 (negative), or 0 (both)." << std::endl;
        return EXIT_FAILURE;
    }


    std::vector<double> candidate_ellipses;
    cv::Mat edge_image;
    std::vector<double> gradient_vectors;
    cv::Mat ls_image;
    cv::Mat* ls_image_ptr = outputLsImagePath.empty() ? nullptr : &ls_image;

    std::cout << "Processing image: " << imagePath << std::endl;

    int result = processImageForEllipses(imagePath, edge_method, polarity,
                                         candidate_ellipses, edge_image, gradient_vectors, ls_image_ptr);

    if (result == 0) {
        // Save outputs
        cv::imwrite("output_edge_image.png", edge_image);
        std::cout << "Edge image saved to output_edge_image.png" << std::endl;

        // Save gradient vectors (example: to a text file)
        FILE* grad_file = fopen("output_gradient_vectors.txt", "w");
        if (grad_file) {
            for (size_t i = 0; i < gradient_vectors.size(); i += 2) {
                fprintf(grad_file, "%.6f %.6f\n", gradient_vectors[i], gradient_vectors[i+1]);
            }
            fclose(grad_file);
            std::cout << "Gradient vectors saved to output_gradient_vectors.txt" << std::endl;
        } else {
             std::cerr << "Warning: Could not open output_gradient_vectors.txt for writing." << std::endl;
        }


        // Save candidate ellipses (example: to a text file)
         FILE* ellipse_file = fopen("output_candidate_ellipses.txt", "w");
        if (ellipse_file) {
             int num_ellipses = candidate_ellipses.size() / 5;
            for (int i = 0; i < num_ellipses; ++i) {
                 fprintf(ellipse_file, "%.2f %.2f %.2f %.2f %.6f\n",
                         candidate_ellipses[i*5], candidate_ellipses[i*5+1],
                         candidate_ellipses[i*5+2], candidate_ellipses[i*5+3],
                         candidate_ellipses[i*5+4]);
            }
             fclose(ellipse_file);
             std::cout << "Candidate ellipses saved to output_candidate_ellipses.txt" << std::endl;
        } else {
            std::cerr << "Warning: Could not open output_candidate_ellipses.txt for writing." << std::endl;
        }


        if (ls_image_ptr) {
            cv::imwrite(outputLsImagePath, *ls_image_ptr);
            std::cout << "Line segment and ellipse image saved to " << outputLsImagePath << std::endl;
        }
