...
def find_distance_fp_to_plane_v2(p1, p2, p3,
                                    angle=math.pi / 2,debug=False) -> float:  # p1 is horizont point for vertical lines,p2 is horizont point for horizontal lines, p3 is nearest point on horizon to center of image. returns distance in pixels
    ...
    if anssq<0:
        try_angle = math.acos(np.dot(p1,p2)/np.linalg.norm(p1)/np.linalg.norm(p2))#recommendation assuming anssq==0
...
def homography_matrix_internal(p1, angle, scale, p3, distance_to_fp,center_pos,only_points=False):
    """
...
def homography_matrix_v2(hor_point1 ,hor_point2 ,center_pos ,only_points = False,angle_between_lines=math.pi/2,debug=False)  :  # ,fov_px,fov_angle):
    ...
    distance = find_distance_fp_to_plane_v2(hor_point1, hor_point2, p3, angle=angle_between_lines,debug=debug)
    angle = angle_of_camera_v1(distance, p3)
    assert (abs(angle) > 1e-5)
    # print(distance, angle)
    return homography_matrix_internal(hor_point1, angle, 0.4, p3, distance,center_pos,only_points=only_points)
